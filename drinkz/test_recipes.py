...
class TestIngredients(object):
    ...
    def test_check_recipes_available(self):
        # Tests the function CheckRecipesAvailable() which checks which 
        # recipes we can make in a given inventory
        ...
        r1 = recipes.Recipe('scotch on the rocks', [('blended scotch', '1000 ml')])
        db.add_recipe(r1)
        ...
        r4 = recipes.Recipe('whiskey bath', [('blended scotch', '5.5 liter')])
        db.add_recipe(r4) 

        recipesAvailable = db.CheckRecipesAvailable()

        testRecipes = set()
        testRecipes.add('scotch on the rocks')
        testRecipes.add('vodka martini')

        assert recipesAvailable == testRecipes, recipesAvailable
